"""`scitex-notification skills` — list / get / install agent-facing skills.

Self-contained. No scitex-dev runtime dep — walks the package's own
`_skills/scitex-notification/` directory directly.
"""
...
PKG = "scitex-notification"


def _skills_root() -> Path:
    """Resolve the bundled `_skills/scitex-notification/` directory."""
    import scitex_notification

    pkg_dir = Path(scitex_notification.__file__).parent
    return pkg_dir / "_skills" / PKG


def _list_skill_files(root: Path) -> list[Path]:
    """All `.md` files under `_skills/scitex-notification/` (recursive), excluding SKILL.md."""
    if not root.is_dir():
        return []
...
def skills_group(ctx) -> None:
    """Agent-facing skills bundled with scitex-notification.

    \b
    Examples:
      $ scitex-notification skills list
      $ scitex-notification skills get 01_installation
      $ scitex-notification skills install                  # → ~/.scitex/dev/skills/scitex-notification/
      $ scitex-notification skills install --claude-symlink # also expose to ~/.claude/skills/scitex/
    """
...
def skills_list(as_json: bool) -> None:
    ...
    Example:
      $ scitex-notification skills list
      $ scitex-notification skills list --json
    """
    root = _skills_root()
...
def skills_get(name: str, as_json: bool) -> None:
    ...
    Example:
      $ scitex-notification skills get 01_installation
      $ scitex-notification skills get 02_quick-start --json
    """
    root = _skills_root()
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Destination dir (default: ~/.scitex/dev/skills/scitex-notification/).",
)
@click.option(
    "--no-link",
    "no_link",
...
def skills_install(
    ...
    \b
    Default: symlink the entire `_skills/scitex-notification/` dir to
    ~/.scitex/dev/skills/scitex-notification/ so add/rename/delete in
    source propagates immediately.

    \b
    Example:
      $ scitex-notification skills install
      $ scitex-notification skills install --claude-symlink
      $ scitex-notification skills install --no-link --dest /tmp/scitex-notification-skills
    """
